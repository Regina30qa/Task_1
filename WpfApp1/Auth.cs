    ...
    public class Auth
    {
        ...
        public string TestPassword(string password)
        {
            ...
            if (password.Length >= 7 && password.Length <= 10)
                ...
                bool IsSpec = false;
                //переменная для проверки идущих подряд одинаковых символов
                bool IsSymbol = false;
                char symbol = ' ';
                //переменная для проверки на  подряд идущих повторяющихся символов
                ...
                foreach (var item in password)
                {
                    ...
                    //проверяем пароль на подряд идущих повторяющихся символов
                    if (symbol == item)
                    {
                        IsSymbol = true;
                    }
                ...
                //Проверяем переменную если истина то есть повторяющиеся символы
                if (IsSymbol)
                {
                    return "В пароле не должно быть подряд идущих повторяющихся символов";
                }
                //Проверяем переменную если истина то есть пробелы
                if (IsWhiteSpace)
                {
